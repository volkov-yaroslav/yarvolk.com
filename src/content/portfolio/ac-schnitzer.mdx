...
date: "2025-12-10"
title: "AC Schnitzer"
description: "WordPress and WooCommerce website for the Norwegian AC Schnitzer distributor, built around product synchronization, used vehicles, and bilingual delivery."
...
There were more advanced ways to solve it. We could have translated the catalog on the server before generating the CSV, using AI translation or services like DeepL, so that Norwegian data was imported as the source of truth. But at 5,000+ products with long descriptions, titles, categories, and metadata, that path would have created a significantly larger cost for the client.

I chose the faster in-house solution instead. Using translation tooling such as **LocoAI**, **Loco Translate**, and **GTranslate**, I set up a workflow where regular pages translated in the usual Norwegian-to-English direction, while product content was handled in reverse so users could still browse the catalog in the language they selected, or based on the browser default.

That approach was not perfect. Because the translation happened client-side, one version would not be indexed as cleanly as a fully server-rendered multilingual setup. But within the budget and timeline, it was the right compromise: the storefront remained usable for real visitors, and the client got a working bilingual experience without funding a much larger translation pipeline.
